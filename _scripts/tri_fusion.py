...
class TriFusion:
    ...
    def tri(self, a, b):
        if b-a > 1:
            ...
            yield from self.change(local_indices, .5)
            yield from self.move_vertical(local_indices[:len(local_indices)//2], local_indices[len(local_indices)//2:])
